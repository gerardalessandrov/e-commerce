...
export default function ContactoPage() {
  ...
  const TU_NUMERO_WHATSAPP = "51900048096";

  const handleSubmit = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (!form.name || !form.phone || !form.message) { toast.error("Por favor completa todos los campos"); return; }
    setSending(true);
    const texto = `🛒 *Nuevo mensaje desde la web*\n\n👤 *Nombre:* ${form.name}\n📱 *Teléfono:* ${form.phone}\n\n💬 *Mensaje:*\n${form.message}`;
    window.open(`https://wa.me/${TU_NUMERO_WHATSAPP}?text=${encodeURIComponent(texto)}`, "_blank");
      ...
      <div className="max-w-5xl mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-2 gap-12">
        <div>
          ...
          <a href="https://wa.me/51999999999" target="_blank" rel="noopener noreferrer"
            className="mt-8 w-full flex items-center justify-center gap-3 bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="space-y-5">
            <div>
              ...
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="María García"
            ...
            <div>
              ...
              <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+51 999 999 999"
                className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-400 text-gray-800" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Mensaje</label>
              <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Hola, quisiera hacer un pedido para..." rows={5}
